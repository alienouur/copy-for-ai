...
**الحل داخل الصفحة (v1.4):** إن وجد الـ Agent أسئلة بحقول إجابة في الصفحة (radio / checkbox / select / حقول نصية / contenteditable) أو أسئلة مرقّمة بدون حقول، يرسلها للخادم بوضع `mode: "fill"` (يعيد JSON منظّماً: `answers[{id, option_ids, text}]`) ويطبّق الإجابات في التبويب نفسه: يختار الخيار، يكتب في الحقل حرفاً حرفاً بأحداث لوحة مفاتيح حقيقية (keydown/beforeinput/input/keyup — فتقبله الحقول التي تمنع اللصق أو تتحقق من الكتابة)، ويضع شارة ✓ بجانب كل سؤال (وللأسئلة بدون حقول تُعرض الإجابة في الشارة). يتقدّم دفعة بعد دفعة (حتى 20 سؤالاً/طلب) حتى ينهي كل الأسئلة، ولا يضغط **Submit** أبداً — راجع ثم أرسل بنفسك. حقول البحث/البريد/الموافقة تُتجاهَل. إن لم يجد أسئلة بحقول يعود للوضع السابق (حل النص/لقطات الشاشة في اللوحة).
